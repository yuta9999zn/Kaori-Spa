...
import { PackagePlus, Plus, Receipt, Wallet, Boxes, Truck, Loader2 } from 'lucide-react';
import {
  useInventoryProducts, useInventoryMoves, recordMove,
  type InventoryProduct, type InventoryMove
} from '@/lib/hooks';

export default function InventoryInView() {
  const t = useTranslations('inventoryIn');
  ...
  const inMoves = (moves ?? []).filter(m => m.moveType === 'in');
  const todayMoves = inMoves.filter(m => m.occurredAt.slice(0, 10) === today).length;
  const totalUnits = inMoves.reduce((s, m) => s + Math.abs(m.delta), 0);
  // TODO(Phase B): supplier + monetary value not on InventoryMove yet —
  // values are placeholders sourced from move counts only.
  const totalValue = 0;
  const suppliers = 0;
  ...
  return (
    <>
      ...
      <section className="grid gap-4 grid-cols-2 xl:grid-cols-4 mb-6">
        <Kpi label={t('kpi.todayReceipts')} value={String(todayMoves)} Icon={Receipt} accent="text-brand-gold" bg="bg-brand-gold/10" />
      ...
      <section className="grid gap-6 lg:grid-cols-3 mb-6">
        <article className="kpi-card lg:col-span-2">
          ...
          <div className="grid gap-3 md:grid-cols-2 mb-4">
            <Field label={t('form.supplier')}>
              {/* TODO(Phase B): supplier not in product schema; placeholder only */}
              <input type="text" placeholder="…" className="spa-input" disabled />
            </Field>
            <Field label={t('form.receiptCode')}>
              {/* TODO(Phase B): receipt code generated server-side */}
        ...
        <article className="kpi-card">
          ...
          <ul className="space-y-3 max-h-[360px] overflow-y-auto">
            {inMoves.length === 0 && (
              <li className="text-xs text-brand-textmuted">Chưa có lịch sử nhập kho</li>
            )}
            {inMoves.slice(0, 20).map((m: InventoryMove) => (
