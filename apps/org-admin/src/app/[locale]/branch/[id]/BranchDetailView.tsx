...
import {
  Store, MapPin, Phone, User, Edit3, Clock, DoorOpen, Users, Sparkles,
  CalendarCog, BarChart2, DollarSign, Calendar, Plus, Crown, Waves, Loader2
} from 'lucide-react';
import { useBranches, type BranchDto } from '@/lib/hooks';
...
// TODO(Phase B): rooms / staff / services come from dedicated endpoints
// (rooms-service, staff-service, catalog) — for now keep the visual mock.
const MOCK_ROOMS = [
...
function pickName(dto: BranchDto): string {
  return dto.name?.vi ?? dto.name?.en ?? dto.code;
}

function pickAddress(dto: BranchDto): string {
  return dto.address?.vi ?? dto.address?.en ?? Object.values(dto.address ?? {})[0] ?? '';
}

export default function BranchDetailView({ branchId }: { branchId: string }) {
  ...
  const branchName = pickName(branch);
  const branchAddress = pickAddress(branch);
  const dayKey = (d: string) => `hours.days.${d}` as 'hours.days.monday';
    ...
    <div className="space-y-8 pb-12">
      ...
      <header className="kpi-card !p-8 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="font-serif text-3xl text-brand-textmain">{branchName}</h1>
            <span className={`rounded-full border px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
              branch.active
                ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                : 'border-slate-200 bg-slate-50 text-slate-500'
            }`}>
              {t(`status.${branch.active ? 'active' : 'active'}` as 'status.active')}
            </span>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:gap-6 text-sm text-brand-textmuted mt-3">
            <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4" /> {branchAddress || '—'}</span>
            <span className="inline-flex items-center gap-2"><Phone className="h-4 w-4" /> {branch.phone ?? '—'}</span>
            {/* TODO(Phase B): manager comes from staff/membership join */}
            <span className="inline-flex items-center gap-2"><User className="h-4 w-4" /> {t('header.manager')}: —</span>
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <section className="kpi-card !p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm">
              <Field label={t('info.fields.name')}     value={branchName} />
              <Field label={t('info.fields.manager')}  value="—" />
              <Field label={t('info.fields.phone')}    value={branch.phone ?? '—'} />
              <Field label={t('info.fields.email')}    value="—" />
              <Field label={t('info.fields.address')}  value={branchAddress || '—'} />
              <Field label={t('info.fields.city')}     value="—" />
              <Field label={t('info.fields.district')} value="—" />
              <Field label={t('info.fields.country')}  value="—" />
            </div>
          </section>
      ...
      <section className="rounded-3xl border border-brand-gold/20 bg-brand-gold/5 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <SnapshotMetric Icon={Calendar}    label={t('snapshot.bookings')} value="—"   delta="" />
          <SnapshotMetric Icon={DollarSign}  label={t('snapshot.revenue')}  value="—"   delta="" />
          <SnapshotMetric Icon={Users}       label={t('snapshot.staff')}    value="—"   delta="" />
        </div>
      </section>
    </div>
  );
