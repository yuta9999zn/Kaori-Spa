      ...
      <div className="overflow-hidden rounded-2xl border border-brand-cream bg-white shadow-soft">
        <table className="w-full text-sm">
          <thead className="bg-brand-cream/40 text-[10px] uppercase tracking-widest text-brand-textmuted">
            <tr>{(['code', 'name', 'plan', 'branches', 'status', 'createdAt'] as const).map(c =>
          ...
          <tbody className="divide-y divide-brand-cream/60">
            {rows.map(r => (
              <tr key={r.code} className="hover:bg-brand-cream/20">
                ...
                <td className="px-4 py-3">{r.branches}</td>
                <td className="px-4 py-3">
                  <span className="rounded-full bg-emerald-50 text-emerald-700 px-2 py-0.5 text-[10px] uppercase">{r.status}</span>
                </td>
                <td className="px-4 py-3 text-brand-textmuted">{r.createdAt}</td>
              </tr>
            ))}
          </tbody>
        </table>
